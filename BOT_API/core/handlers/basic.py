...
Func:
    ...
    get_history_price_step_one: Получение истории цен на товар.
        FSM первый этап.
    get_history_price_step_two: Получение истории цен на товар.
        FSM второй этап.
"""
import os
import logging
...
async def form_add_item(message: Message, state: FSMContext) -> None:
    """
...
async def form_url_info(message: Message, state: FSMContext) -> None:
    """
...
async def form_url_price(message: Message, state: FSMContext) -> None:
    """
    ...
    try:
        ...
        if 'detail' in response:
            ...
            logger.debug(f"Проблема с форматом введённой почты! {error}")
        else:
...
async def delete_step_one(message: Message, state: FSMContext) -> None:
    """
...
async def delete_step_two(message: Message, state: FSMContext) -> None:
    """
    ...
    try:
        ...
        if 'detail' in response:
            ...
            logger.debug(
                f"Ошибка добавления товара: {error['type']}, {error['msg']}")
        else:
            await message.answer(response['message'])
    except aiohttp.ClientError as ex:
        await message.answer(
            "Ошибка соединения с сервером... Попробуйте позже.")
        logger.debug(f"Ошибка соединения {ex}")
    finally:
        await state.clear()


@router.message(F.text == emoticons[4], F.from_user.id == admin_id)
async def get_list_monitoring(message: types.Message) -> None:
    """
...
async def get_history_price_step_one(message: Message,
...
async def get_history_price_step_two(message: Message,
                                     state: FSMContext) -> None:
    """
    ...
    try:
        ...
        if 'detail' in response:
            ...
            if history_price == []:
                await message.answer(
                    "Первая цена добавиться в течении 5 минут!")
            if isinstance(history_price, list):
                for item in history_price:
                    time_obj = datetime.strptime(item['date'],
                                                 "%Y-%m-%dT%H:%M:%S.%f")
